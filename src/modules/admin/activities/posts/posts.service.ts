...
import {
  serializeAdminActivityPost,
  type AdminActivityPostItem,
} from './posts.serializer';

@Injectable()
export class AdminActivityPostsService {
  constructor(
  ...
  async list(
  ...
  ): Promise<CursorPage<AdminActivityPostItem>> {
    ...
    return {
      items: page.map((row) => serializeAdminActivityPost(this.storage, row)),
